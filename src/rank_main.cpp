...
string modelfile = "~/data/model_linear";
string img_save_dir = "~/projectroot/Data/results_img/";
...
int main(int argc, char *argv[]) {
	...
	for(int xyz = 1; xyz < 2; xyz ++)
	{
		string number = "000" + convertInt(xyz);
		if(xyz > 9)
			number = "00" + convertInt(xyz);
		if(xyz > 99)
			number = "0" + convertInt(xyz);
		string valstr = string(argv[1]);
		string numS = string(argv[2]);

		string imgFile = string(argv[1]);
		string bgFile = string(argv[2]);
		string pcdFile = string(argv[3]);
		...
		if (SAVE_RECTS) {
			char name[500];
			sprintf(name, "%skam_best_rectangles_%s.txt",img_save_dir.c_str(),filenum.c_str());
