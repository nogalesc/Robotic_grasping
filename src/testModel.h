...
printf("Starting main for loop \n");
	for (unsigned int c = 0; c < angles.size(); c++) {
		...
		pool = thr_pool_create(5, 5, 0, NULL);
		...
		printf("Checking point cloud info \n");
		if (USE_POINT_CLOUD) {
		...
		printf("image features histogram \n");
		if (USE_IMAGE_FEATURE) {
      ...
      maxVal = (maxVal < minVal ? minVal + binWidth : maxVal);
			...
			////printf("point cloud first for loop \n");
			for (int i = 1;i < nBinsTotal;i++) {
...
////printf("point cloud first for loop \n");
      ...
      maxVal = (maxVal < minVal ? minVal + binWidth : maxVal);
		...
		//printf("Cleaning up image features \n");
		if (USE_IMAGE_FEATURE) {
			delete[] ranges;
