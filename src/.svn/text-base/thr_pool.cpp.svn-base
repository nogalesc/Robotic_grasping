...
create_worker(thr_pool_t *pool)
{
	...
	if(thread == NULL)
		cout << "Failed to MALLOC a thread " << endl;
	...
	if(error)
		cout << "Failed to pthreadCreate" << endl;
	return (error);
}
 ...
 */
static void
worker_cleanup(void* in)
{
    thr_pool_t *pool = (thr_pool_t*) in;
	--pool->pool_nthreads;
...
thr_pool_wait(thr_pool_t *pool)
{
	...
	while (pool->pool_head != NULL || pool->pool_active != NULL) {
		pool->pool_flags |= POOL_WAIT;
		//cout << "calling pthread_cond_wait" << endl;
